...
#define GTK_FZ1B_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_FZ1B_KNOB, GtkFz1bKnobPrivate))

G_DEFINE_TYPE(GtkFz1bKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkFz1bKnobPrivate GtkFz1bKnobPrivate;

struct _GtkFz1bKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkFz1bKnobPrivate *priv = GTK_FZ1B_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkFz1bKnob *knob = GTK_FZ1B_KNOB(widget);
...
{
    g_assert(GTK_IS_FZ1B_KNOB(widget));
    GtkFz1bKnob *knob = GTK_FZ1B_KNOB(widget);
    GtkFz1bKnobPrivate *priv = GTK_FZ1B_KNOB_GET_PRIVATE(knob);
...
{
    GtkFz1bKnob *knob = GTK_FZ1B_KNOB(object);
...
/****************************************************************
 ** init the GtkFz1bKnobClass
 */

static void gtk_knob_class_init (GtkFz1bKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkFz1bKnobPrivate));
}
...
static void gtk_knob_init (GtkFz1bKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_FZ1B_KNOB, NULL ));
