...
static void resize_event(gx_maestro_fz1bUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_maestro_fz1bUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_maestro_fz1bUI* ui = (gx_maestro_fz1bUI*)malloc(sizeof(gx_maestro_fz1bUI));
	...
	ui->controls[0] = (gx_controller) {{ 1.0, 1.0, 0.0, 1.0, 1.0},{ 35, 60, 61, 61}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 60, 61, 61}, false,"BALANCE", KNOB, BALANCE};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 205, 60, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_maestro_fz1bUI* ui = (gx_maestro_fz1bUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_maestro_fz1bUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_maestro_fz1bUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_maestro_fz1bUI *ui, gx_controller* controller) {
...
static void _expose(gx_maestro_fz1bUI *ui) {
	static const char* plug_name = "GxFz1b" ;
	...
	cairo_scale (ui->cr, 1.9, 2.85);
	cairo_set_source_surface (ui->cr, ui->frame1, 10, 12);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/1.9, 1/2.85);
...
static void controller_expose(gx_maestro_fz1bUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_maestro_fz1bUI *ui, int controller) {
...
static void check_value_changed(gx_maestro_fz1bUI *ui, int i, float* value) {
...
static void check_is_active(gx_maestro_fz1bUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_maestro_fz1bUI *ui) {
...
static bool get_active_ctl_num(gx_maestro_fz1bUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_maestro_fz1bUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_maestro_fz1bUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_maestro_fz1bUI *ui, int i) {
...
static void button1_event(gx_maestro_fz1bUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_maestro_fz1bUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_maestro_fz1bUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_maestro_fz1bUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_maestro_fz1bUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_maestro_fz1bUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_maestro_fz1bUI *ui, bool set) {
...
static void event_handler(gx_maestro_fz1bUI *ui) {
	XEvent xev;
