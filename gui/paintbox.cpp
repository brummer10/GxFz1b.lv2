...
G_DEFINE_TYPE(GxFz1bPaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxFz1bPaintBoxClass *klass)
{
...
static void set_paint_func(GxFz1bPaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxFz1bPaintBox *paint_box = GX_FZ1B_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxFz1bPaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxFz1bPaintBox *paint_box = GX_FZ1B_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_FZ1B_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxFz1bPaintBox *paintbox = GX_FZ1B_PAINT_BOX(wi);
